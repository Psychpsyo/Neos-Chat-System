...
async def createNewRoom(name, icon, userID, bySystem = False):
	...
	async with roomLock:
		...
		# create the room
		lastRoomID += 1
		rooms.append({"id": lastRoomID, "name": "<noparse=" + str(len(name)) + ">" + name, "users": [] if bySystem else [socket.get()] , "owner": userID, "messages": [], "icon": icon, "alwaysOpen": True if bySystem else False})
...
def formatRichMessage(message):
	...
	for part in newMessage:
		...
		if currentCode:
			# we have reached an RTF tag, insert the noparse and append to message
...
async def sendMessage(message):
	...
	message = formatRichMessage(message)
